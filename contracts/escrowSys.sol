...
contract escrowSystem {
    ...
    function pay(address judgeaddr, uint256 deposits)
        public
        payable
        returns (bool)
    {
    ...
    function withdrawWorker(uint256 idContract) public returns (bool) {
        ...
        contractsDetails[idContract].worker.transfer(
            contractsDetails[idContract].deposit
        );
        contractsDetails[idContract].worker.transfer(
            contractsDetails[idContract].price
        );
        return true;
    }
    ...
    //@return return true if function work fine
    function withdrawEmployer(uint256 idContract)
        public
        payable
        returns (bool)
    {
        ...
        contractsDetails[idContract].employer.transfer(
            contractsDetails[idContract].price
        );
        contractsDetails[idContract].employer.transfer(
            contractsDetails[idContract].deposit
